...
import StatusBitcoinTransactionSent from 'blindmixer-lib/dist/status/bitcoin-transaction-sent';
import StatusFailed from 'blindmixer-lib/dist/status/failed';
...
export default async function sendFeeBump(feebump: hi.FeeBump) {
  ...
  const isblindmixer = await rpcClient.getTransaction(oldTxid);

  if (isblindmixer === undefined) {
    throw `cannot determine transaction ${oldTxid}`;
  }

  const hasSize = await rpcClient.decodeRawTransaction(isblindmixer.hex);
